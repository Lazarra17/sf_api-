...
const SubCategoryController = () => {
  const index = async (req, res) => {
    try {
      const categories = await sequelize.Sub_categories.findAll({
        order: [['id', 'DESC']],
  ...
  const store = async (req, res) => {
    ...
    let category = await sequelize.Sub_categories.create({
      identifier,
      title: form.title,
    });
  ...
  const update = async (req, res, next) => {
    ...
    const category = await sequelize.Sub_categories.findOne({
      where: { id },
      attributes: ['id', 'identifier', 'title'],
    });
