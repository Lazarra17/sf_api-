...
const initLocalVariables = (app) => {
  ...
  const server = `${(config.secure && config.secure.credentials ? 'https://' : 'http://') + process.env.NODE_HOST}:${process.env.PORT}`;
...
const initStaticRoutes = (app) => {
  app.use('/', express.static(path.resolve('./src/storage'), { maxAge: 86400000 }));
};
...
const initRoutes = (app) => {
  ...
  app.use('/auth', publicMappedRoutes);

  privateMappedRoutes = mapRoutes(privateRoutes, 'src/app/controllers/', [authMiddleware]);
